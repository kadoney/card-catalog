...
export const useFLIPAnimation = (gridRef: RefObject<HTMLDivElement>) => {
  ...
  const animateToNewPositions = useCallback(async () => {
    if (!gridRef.current || animatingRef.current) return;
    ...
    oldIds.forEach((id) => {
      if (!newIds.has(id)) {
        const el = gridRef.current?.querySelector(`[data-card-id="${id}"]`) as HTMLElement;
        if (el) {
    ...
    cards.forEach((el) => {
      ...
      if (dx === 0 && dy === 0) return; // No movement needed
